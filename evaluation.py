...
#         # print(f"Epoch   {epoch}, F1-Score   {fsc:.3f}, Roc  {auc:.3f}, mcc {mcorr:.3f}")
#     return {
...
def compute_classifier_metrics(probs, labels, lengths, epoch, file_name, threshold=None, cv=None, config=None):
    ...
    print("pre", *l_pred.tolist())
    m_corr = matthews_corrcoef(efective_labels, l_pred)
    rec = recall_score(efective_labels, l_pred)
    pre = precision_score(efective_labels, l_pred)
    ...
    if epoch == 'test':
        ...
        f.write(f"ROC-AUC = {auc:.3f}\n")
        f.write(f"PR-AUC = {aupr:.3f}\n")
        f.write(f"MCC = {m_corr:.3f}\n")
        f.close()

        print(f"Epoch   {epoch}, Recall    {rec:.3f}, Precision  {pre:.3f}, F1-Score   {fsc:.3f}, Roc  {auc:.3f}, PR   {aupr:.3f}, MCC {m_corr:.3f}")
